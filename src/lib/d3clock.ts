...
import { IClockConfig, IClockData, IClockFace, DateFn } from '..';
...
export const d3clock = (config: IClockConfig) => {
  ...
  const fields = (date?: string | number | DateFn) => {
    const d = date
  ...
  if (config.date && (typeof config.date === 'string' || typeof config.date === 'number')) {
